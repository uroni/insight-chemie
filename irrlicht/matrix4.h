	...
	*/
	class matrix4
	{
		public:
			...
			void matrix4::buildRotationMatrix(core::vector3df from, core::vector3df to, f32 rate) {
				f32 dotp = from.dotProduct(to);
				...
				if (dotp == 1.0f) {
					...
					if (dotp == -1.0f) {
						axis.set(from.Y, from.Z, from.X);
					} else {
						axis = from.crossProduct(to);
						axis.normalize();
					}
					f32 angle = acos(dotp) * rate;
					buildRotationMatrix(angle, axis);
				}
			}

			void matrix4::buildRotationMatrix(f32 phi, core::vector3df axis) {
				core::matrix4 matrix;
